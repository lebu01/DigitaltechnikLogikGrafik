...
function generateLogic(inputVar, canvasVar, outputVar){
    ...
    var offsetInitialX = 0;
    var offsetInitialY = 0;
    ...
    function main(inp, can){
        ...
        var initialPos = {
            x: FRAMEWIDTH - 2*sizeOutput/* - distBetweenX*/ - padding.right,
        ...
        // Redo
        var tempHighest = FRAMEHEIGHT - highestBoxY - /*padding.top +*/ padding.bottom + 2*padding.top;
        if(cornersAtLines.length == 0) tempHighest = FRAMEHEIGHT - highest - padding.bottom + 2*padding.top;
        //var tempHighest = FRAMEHEIGHT - padding.top - padding.bottom - highest/* + padding.top + padding.bottom*/;
        var tempFarest = FRAMEWIDTH - farestObjectX /* padding.left*/ - padding.right;
        //var tempFarest = FRAMEWIDTH - padding.left - padding.right - farestObjectX/* + padding.left + padding.right*/;
        ...
        //console.error('REDO: fX: '+farestX);
        //console.error('REDO: fOX: '+farestObjectX);
        //console.error('Farest: '+tempFarest);

        negationDotSize = 4;
        var b = calcScaling(tempFarest, tempHighest);

        resetGlobals();

        initialPos = {
            x: FRAMEWIDTH - 2*sizeOutput - padding.right,
            y: FRAMEHEIGHT-size.height - padding.bottom
        }

        //hightest = initialPos.y;
        highestBoxY = initialPos.y;

        farestX = FRAMEWIDTH;
        farestObjectX = FRAMEWIDTH;
        ...
        // REDO für mittige Positionierung **************************************
        //console.clear();
        ...
        if(b){
            offsetInitialY = (FRAMEHEIGHT - (FRAMEHEIGHT - highest)) / 2;
        }else {
            offsetInitialX = (FRAMEWIDTH - (FRAMEWIDTH - farestObjectX)) / 2;
        }
        //console.error('OffX: '+offsetInitialX+'; OffY: '+offsetInitialY);

        resetGlobals();

        initialPos = {
            x: FRAMEWIDTH - 2*sizeOutput - padding.right - offsetInitialX,
            y: FRAMEHEIGHT-size.height - padding.bottom - offsetInitialY
        }
    ...
    function calcScaling(x=0, y=0){
        ...
        if(x == 0 && y == 0){
            ...
            maxTreeWidth = remainingDepth(array)+3;
            ...
            xScaling = (FRAMEWIDTH - padding.left - padding.right) / (maxTreeWidth * size.width + sizeInputBox.width + sizeOutput*2);
        }else {
        ...
        if(xScaling < yScaling){
            ...
            negationDotSize = Math.round(negationDotSize * xScaling);
            if(negationDotSize < 2) negationDotSize = 2;
            if(negationDotSize > distBetweenX) negationDotSize = distBetweenX-2;
            return true;
        }else {
            ...
            negationDotSize = Math.round(negationDotSize * yScaling);
            if(negationDotSize < 2) negationDotSize = 2;
            if(negationDotSize > distBetweenX) negationDotSize = distBetweenX-2;
            return false;
        }
    ...
    function resetGlobals(all = false){
        if(all){
            ...
            farestX = FRAMEWIDTH;
            farestObjectX = FRAMEWIDTH;
            sizeInputBox.height = 16;
            sizeInputBox.width = 16;
            sizeOutput = 8;
            distBetweenX = 15;
            distBetweenY = 10;
    ...
    function drawArrayData(stage, arr, neg, parentPosition = {x: FRAMEWIDTH, y: FRAMEHEIGHT}, parentInputs = [], amountParentInputs = 0){
        ...
        while(i < arr.length){
            if(arr[i] == '!'){ // Abfrage ob gesamter Block negiert
                ...
                if(Array.isArray(arr[i])){
                    ...
                    if(highest > boxFrom.y) highest = boxFrom.y;
                    ...
                    //if(remainingDepth(arr) == 1) highestBoxY = boxFrom.y;
                    ...
                    for(var j = arr[i+1].length-1; j >= 0; j--){
                        ...
                        if(Array.isArray(arr[i+1][j]) && arr[i+1][j].length == 2 && arr[i+1][j][0] == '!' && !Array.isArray(arr[i+1][j][1])){
                            ...
                            var negDot = {
                                x: boxLineTo.x-negationDotSize, // Abzug des Radius des Kreises
                                y: boxLineTo.y
                            }
                            drawCircle(negDot, negationDotSize, true);
                        }
                    ...
                    if(stage > 0 && parentInputs.length > 0){ // Verbindungslinien zeichnen
                    ...
                    if(Array.isArray(arr[i+1]) && (arr[i+1].length > 2 || arr[i+1][0] != '!')){
                        drawArrayData(stage+1, arr[i+1], false, boxFrom, inputs, actualAmountInputs);
                    }/*else if(!Array.isArray(arr[i+1]) && inputVars.includes(arr[i+1])){
                        drawArrayData(stage+1, arr[i+1], false, boxFrom, parentInputs, actualAmountInputs);
                    }*/
                ...
                */
                ...
                for(var j = 0; j < getInputConnectionsOfInput(inputVarsAmount[i][0]).length; j++){
                    ...
                    if(farestX < getInputConnectionsOfInput(inputVarsAmount[i][0])[j][2].x && checkStraightLineBoxCollision(posDot, getInputConnectionsOfInput(inputVarsAmount[i][0])[j][2]) == true){ // Überprüfen ob Input von Block in oberster Stage liegt -> hier nicht in oberster Stage
                        ...
                        if(highestBoxY > hY) highestBoxY = hY;
                ...
                var posDot = {
                    x: getInputConnectionsOfInput(inputVarsAmount[i][0])[0][1].x,
                    y: getInputConnectionsOfInput(inputVarsAmount[i][0])[0][2].y
                }

                if(farestX < getInputConnectionsOfInput(inputVarsAmount[i][0])[0][2].x && checkStraightLineBoxCollision(posDot, getInputConnectionsOfInput(inputVarsAmount[i][0])[0][2]) == true){

                    var hY = highest-distBetweenY;
                    while(usedConnectionPathsX.includes(hY)){
                        hY -= distBetweenY;
                    }
                    var dest = getInputConnectionsOfInput(inputVarsAmount[i][0])[0][2];
                    const realDest = Object.assign({}, dest); // Object assign wegen referenzierungsfehler
                    ...
                    var from = getInputConnectionsOfInput(inputVarsAmount[i][0])[0][1];
    ...
    function drawInputs(){
        ...
        //console.error('Highest-y: '+highestBoxY);

        //drawLine({x: 0, y: highest}, {x: 50, y: highest});

        if(highest < highestBoxY) highest = highestBoxY;
        if(highest < padding.top) highest = 2*padding.top;
        //console.error('Highest: '+highest);
        ...
        for(var i = 0; i < arr.length; i++){
            ...
            //var yCord = FRAMEHEIGHT - padding.bottom - sizeBox.height - (i * (FRAMEHEIGHT/*-padding.bottom*/-padding.top)/arr.length); // *0.5 für Abstand oben
            //var maxPos = FRAMEHEIGHT - padding.top - padding.bottom;
            var maxPos = FRAMEHEIGHT - padding.bottom /+ padding.top - t;
            if(xScaling < yScaling) maxPos = FRAMEHEIGHT - highest/* - padding.bottom*/;

            var yCord = 0;
            if((maxPos - (arr.length * sizeBox.height)) <= 0){
                yCord = FRAMEHEIGHT - padding.bottom - offsetInitialY/2 - sizeBox.height - i * (sizeBox.height + 10);
            }else {
                yCord = FRAMEHEIGHT - padding.bottom - offsetInitialY/2 - sizeBox.height - i * (sizeBox.height + ((maxPos - (arr.length * sizeBox.height)) / (arr.length - 1)));
            }
            ...
            const lineTo = {
                x: from.x + sizeBox.width + distBetweenX,
                y: from.y + sizeBox.height/2
            }
        ...
        if(tempX < farestObjectX) farestObjectX = tempX;
        //console.error('Farest Object X: '+farestObjectX);
        //console.error('From x: '+tempX);
    }
    ...
    function getAmountOfBlockInputsOnStage(stageVar, a){
        ...
        function traverse(node, stage = 0) {
            ...
            //console.error('NOde: '+JSON.stringify(node)+'; '+neg+'; '+stage);
            ...
            if(node[0] != '!'){
                ...
                if(Array.isArray(node) && operands.includes(node[0])) totalInputsPerStage[stage] += node[1].length;
                if(operands.includes(node[0]) && remainingDepth(node) > 1) stage++;
            }
            ...
            for (let child of node) {
                if(Array.isArray(child) && !operands.includes(child[0])){
                    ...
                    continue;//traverse(node[1], stage, true);
                }else {
                    continue;//traverse(child, stage);
                }
            }
        }
        ...
        //for (let root of a) {
            traverse(a, 0);
        //}
        //console.error('Full: '+JSON.stringify(totalInputsPerStage));
